...
class MainModel extends CI_Model
{
    ...
    function getBatchDetails($tb_id){
        $this->db->select('tbl_training_batches.*,trade_code,trade_name,scheme_name,subscheme_name,optional_exam_type'); 
    ...
    function getBatchCandidateDetails($tb_id){
        ...
        $this->db->select("student_id,enrollment_number,COALESCE(student_name, '') as student_name,COALESCE(password, '') as password,profile_verification_status,practical_activity_questions,viva_questions,
                            practicalactivity_video_file,viva_video_file,practicalactivity_answers_record_generated,viva_answers_record_generated,theory_submission_dts,
                            ...
                            IF(aadhar_front_filename != '', CONCAT('$aadhaar_filename_path', aadhar_front_filename), '') AS aadhar_front_filename,
                            IF(aadhar_back_filename != '', CONCAT('$aadhaar_filename_path', aadhar_back_filename), '') AS aadhar_back_filename,
                            IF(student_photo_with_aadhar != '', CONCAT('$aadhaar_filename_path', student_photo_with_aadhar), '') AS student_photo_with_aadhar");
        $this->db->join('tbl_trades', 'tbl_trades.trade_id = tbl_training_batches.trade_id');	
        $this->db->join('tbl_schemes', 'tbl_schemes.scheme_id = tbl_training_batches.scheme_id');	
        $this->db->join('tbl_subschemes', 'tbl_subschemes.subscheme_id = tbl_training_batches.subscheme_id');	 
        $this->db->join('tbl_students', 'tbl_students.tb_id = tbl_training_batches.tb_id','LEFT');
    ...
    function getCandidateBatchDetails($student_id){
        $this->db->select('tbl_students.*,lid,practical_activity_instructions,viva_instructions,batch_id'); 
    ...
    public function getCandidateMarkedQuestionCount($tb_id,$tableName)
	{      
		$this->db->select('student_id,count(qid) as total_qns_marked');  
		$this->db->from($tableName);
        $this->db->where('tb_id',$tb_id);
        $this->db->where('assessor_id > 0');
    ...
    public function getCandidatePendingQuestionCount($student_id,$tableName)
	{      
		$this->db->select('count(qid) as total_qns_pending');  
		$this->db->from($tableName);
        $this->db->where('student_id',$student_id);
        $this->db->where('assessor_id = 0');
        $this->db->group_by('student_id');
        ...
        $result=$query->result_array();
    	if(count($result)>0){
    		return $result[0]['total_qns_pending'];
    	}else{
    	    return 0;
    	}
    }

    public function getCandidatePendingQuestionCountByBatch($tb_id,$tableName)
	{      
		$this->db->select('count(qid) as total_qns_pending');  
		$this->db->from($tableName);
        $this->db->join('tbl_students', "tbl_students.student_id = {$tableName}.student_id");
        $this->db->where("{$tableName}.tb_id", $tb_id); 
        $this->db->where('assessor_id = 0');
    ...
    public function getCandidateAssessmentStatusPendingCount($tb_id)
	{      
		$this->db->select('count(student_id) as total_students_pending');  
		$this->db->from('tbl_students');
        $this->db->where('tb_id',$tb_id);
        $this->db->where('student_attendance','Present');
        $this->db->where_not_in('student_assessment_status', array('Pending','Completed')); 
        $this->db->group_by('tb_id');
        //$this->db->order_by('student_id','ASC');
        $query = $this->db->get();
        //echo "<br> str ".$this->db->last_query();exit;
        $result=$query->result_array();
    	if(count($result)>0){
    		return $result[0]['total_students_pending'];
    	}else{
    	    return 0;
    	}
    }

    public function getBatchChecklistDocumentsUploadedCount($tb_id)
	{      
		$this->db->select('count(tcdd_id) as uploaded_checklist_documents_count');  
		$this->db->from('tbl_checklist_documents_details');
        ...
        $this->db->where('tb_id',$tb_id);
        $this->db->where('document_requirement','Mandatory');
        $query = $this->db->get();
        //echo "<br> str ".$this->db->last_query();exit;
        $result=$query->result_array();
    	if(count($result)>0){
    		return $result[0]['uploaded_checklist_documents_count'];
    	}else{
    	    return 0;
    	}
    }
    
    function getAssessorBatchCandidateAttendenceDetails($assessor_id,$tb_id = 0){
        $this->db->select('tbl_students.tb_id,count(student_id) as pending_attendance'); 
        $this->db->join('tbl_training_batches', 'tbl_training_batches.tb_id = tbl_students.tb_id');	
        $this->db->where('tbl_training_batches.assessor_id', $assessor_id);
        $this->db->where('tbl_training_batches.tb_assessment_status', 'Pending');
        $this->db->where('tbl_students.student_attendance', 'Pending');
        if($tb_id > 0) {
            $this->db->where('tbl_training_batches.tb_id', $tb_id);
        }
    ...
    public function getAssessorBatchChecklistDocumentsUploadedCount($assessor_id)
	{      
		$this->db->select('count(tcdd_id) as uploaded_checklist_documents_count,tb_id');  
		$this->db->from('tbl_checklist_documents_details');
        $this->db->join('tbl_assessment_checklist_documents_master','tbl_assessment_checklist_documents_master.acdm_id = tbl_checklist_documents_details.acdm_id');
        $this->db->where('assessor_id',$assessor_id);
        $this->db->where('document_requirement','Mandatory');
    ...
    public function getAnswerDetailsByCandidateId($candidate_id)
	{      
		$this->db->select('*');  
		$this->db->from("tbl_theory_answers");
        $this->db->where('student_id',$candidate_id);
    ...
    public function getExpenseDetailsByBatch($tb_id)
	{      
		$this->db->select('*');  
		$this->db->from("tbl_batch_expenses");
    ...
    public function getTotalTravelChargesByBatch($be_id)
	{      
		$this->db->select('SUM(travel_amount) + SUM(lunch) + SUM(breakfast) + SUM(dinner) + SUM(hotel_stay) as total_travel_charges');  
		$this->db->from("tbl_batch_expense_details");
		$this->db->where('be_id',$be_id);
        $this->db->group_by('be_id'); 
        $query = $this->db->get();
        //echo "<br> str ".$this->db->last_query();exit;
        $result=$query->result_array();
    	if(count($result)>0){
    		return $result[0]['total_travel_charges'];
    	}else{
    	    return 0;
    	}
    }
