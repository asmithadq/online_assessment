...
class Api_assessor extends MY_Controller
{
    ...
    public function watermarkImage($watermarkValue,$file_path)
    {
        ...
        $overlay_height = 650; // Height of the overlay (adjust as needed)
    ...
    public function login(){
        ...
        //if ($userRes['status'] == true) {
            ...
            $arr_user = $this->mainModel->getAllRecords('tbl_assessor',array("assessor_code" => $assessor_code,"assessor_password" => $password, "assessor_status" => 1));
            
            if($arr_user == false){ //Invalid User
                $data['status'] = false;
                $data['rcode'] = 500;
                $data['message'] = 'Invalid Assessor';
            }
    		else{  
        ...
        if ($userRes['status'] == true) {
            ...
            if ($tokenRes['status'] == true) {
                if($this->input->post('image') != "" && $this->input->post('image_type') != "" && $this->input->post('tb_id') > 0 
                    ...
                    $batch_id = str_replace("/","-",$arr_batch[0]['batch_id']);
            ...
            if ($tokenRes['status'] == true) {
                ...
                if($student_id > 0 && $type != "" && isset($_FILES['video_file']) && $this->input->post('geo_address') != "" && $this->input->post('lat') != "" && $this->input->post('long') != "") {
                    if(isset($_FILES['video_file']) && $_FILES['video_file']['error'] == 0) {
                        // File paths
                        $input_file = $_FILES['video_file']['tmp_name'];
                        ...
                        $data['status'] = true;
                        $data['rcode'] = 200;
                        $data['message'] = ($output == 'Success') ? 'Video saved successfully' : 'Error watermarking Video';  
                    }
                    else {
                        $data['status'] = false;
                        $data['rcode'] = 500;
        ...
        if ($userRes['status'] == true) {
            ...
            if ($tokenRes['status'] == true) {
                if($this->input->post('acdm_id') != "" && $this->input->post('document_type') != "" && $this->input->post('tb_id') > 0 && $this->input->post('document_title') != ""
                    ...
                    if($this->input->post('document_type') == 'file') {
                    ...
                    else if($this->input->post('document_type') == 'image') {
        ...
        if ($userRes['status'] == true) {
            ...
            if ($tokenRes['status'] == true) {
                ...
                if ($arr_assessor_expenses_list != false) {
                    ...
                    $data['paid_receipt_file'] = ($arr_assessor_expenses_list[0]['paid_receipt_file'] != "") ? $arr_assessor_expenses_list[0]['paid_receipt_file'] : "";
                    $data['paid_date'] = ($arr_assessor_expenses_list[0]['paid_date'] != "") ? $arr_assessor_expenses_list[0]['paid_receipt_file'] : "";
                }
                else {
                    $data['status'] = false;
                    $data['rcode'] = 500;
    ...
    public function saveBatchExpenseFile()
    {
        ...
        if ($userRes['status'] == true) {
            ...
            $arr_batch_details = $this->mainModel->getBatchDetails($tb_id);
            if($arr_batch_details != false) {
                $batch_id = str_replace("/","-",$arr_batch_details[0]['batch_id']);
            }
            ...
            if ($tokenRes['status'] == true) {
                if($tb_id > 0 && $ted_id > 0) {
                    if($document_file_type == 'file') {
                        if(isset($_FILES['file'])) {
                            $insDetailsData[$document_name] = $this->uploadFile('file', 'assessors_expenses', $batch_id.'-'.$be_id.'-'.$document_name.'-'.date('dmYHis'));
                        }  
                    }
                    else if($document_file_type == 'image') {
                        $insDetailsData[$document_name] = $this->uploadBase64ImageNoThumb($this->input->post('file'), 'assessors_expenses', $batch_id.'-'.$be_id.'-'.$document_name.'-'.date('dmYHis'));
                    }
