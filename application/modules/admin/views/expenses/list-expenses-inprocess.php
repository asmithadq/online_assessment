    ...
    buttons: [
        {
            ...
            action: function (e, dt, node, config) {
                ...
                $.ajax({
                    "url": '<?php echo base_url('export-expenses-inprocess'); ?>/0/NULL/NULL/Submitted',
                    type: 'POST',
                    //data: { length: '-1', param2: 'value2' },
                    success: function(response) {
                        window.location.href = '<?php echo base_url('export-expenses-inprocess'); ?>/0/NULL/NULL/Submitted';
                    },
...
$('#btn_save').click(function(e) {
    ...
    var table = $('#serverSideDataTable').DataTable({
        ...
        buttons: [
        {
            ...
            action: function (e, dt, node, config) {
                ...
                // Perform your AJAX call here
                $.ajax({
                    "url": '<?php echo base_url('export-expenses-inprocess'); ?>/'+assessor_id+'/'+start_date+'/'+end_date+'/Submitted',
                    type: 'POST',
                    //data: { length: '-1', param2: 'value2' },
                    success: function(response) {
                        window.location.href = '<?php echo base_url('export-expenses-inprocess'); ?>/'+assessor_id+'/'+start_date+'/'+end_date+'/Submitted';
                    },
                    error: function(xhr, status, error) {
                        // Handle error
                    }
    ...
    table.on('xhr.dt', function (e, settings, response, xhr) { 
        ...
        $('#hdn_assessor_id').val(assessor_id);
        $('#hdn_start_date').val(start_date);
        $('#hdn_end_date').val(end_date);
    });
