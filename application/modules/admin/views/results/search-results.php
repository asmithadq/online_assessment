				...
				<?php 
					...
					<div class="card">
						<div class="card-header">
							...
							<div class="bootstrap-badge div_search_results" style="display:none;"> 
						...
						<div class="card-body">
							<div class="basic-form">
								<form class="needs-custom-validation" novalidate id="myForm" method="post" action="">
									...
									<input name="hdn_batch_id" id="hdn_batch_id" type="hidden" value="<?php echo $batch_id; ?>">
									<div class="row">
										<div class="mb-3 col-md-3">
											<label class="form-label">Sectorskill Council</label>
											<select id="ssc_id" name="ssc_id" class="form-control">
										...
										<div class="mb-3 col-md-3">
											...
											<select class="form-control" name="trade_id" id="trade_id">
												<option value="">-Select-</option>
											</select>
											<div class="invalid-feedback"> Please select Trade/QP Name. </div>
										</div>
										<div class="mb-3 col-md-3">
											<label class="form-label">Batch ID </label> <span class="text-danger">*</span>
											<select class="form-control results_batch_id" name="batch_id" id="batch_id" required>
												<option value="">-Select-</option>
											</select>
											<div class="invalid-feedback"> Please select Batch ID. </div>
										</div>
									</div>
									<button type="button" name="submit" class="btn btn-primary" id="btn_save">Get Results</button>
								</form>
							</div>
						</div>
					</div>
				</div>
				<div class="col-xl-12 col-lg-12 div_search_results" style="display:none;">
...
$('#btn_save').click(function(e) {
	...
	var error = "";
	/*if(trade_id == "") {
		error += "Please select Trade <br>";
	}
	if(ssc_id == "") {
		error += "Please select Sectorskill Council <br>";
	}*/
	if(batch_id == "") {
		error += "Please select Batch <br>";
	}
	if(error != "") {
...
function get_trades() {
	...
	// AJAX request
	$.ajax({
		url: "<?php echo base_url('get-results-trades-by-ssc'); ?>",
		method: 'post',
		data: {
			ssc_id: ssc_id
		},
		...
		success: function(response) {
			...
			$("#batch_id").html('<option value="">Choose...</option>');
		}
	});
...
function get_batches() {
    var ssc_id = $("#ssc_id option:selected").val();
	var trade_id = $("#trade_id option:selected").val();
	var batch_id = $('#batch_id option:selected').val();
	var hdn_batch_id = $("#hdn_batch_id").val();
	if(batch_id > 0) {
		hdn_batch_id = batch_id;
	}
	...
	//if(trade_id > 0) {
		// AJAX request
		$.ajax({
			url: "<?php echo base_url('get-batch-by-ssc-trade'); ?>",
			method: 'post',
			data: {
				ssc_id: ssc_id,
				trade_id: trade_id,
				hdn_batch_id : hdn_batch_id
			},
			dataType: 'json',
			success: function(response) {
				//console.log('output '+response.output); 
				// Update CSRF hash
				$("#batch_id").html(response.output);
				if (hdn_batch_id > 0) {
